...
export const RepoRemoteStatusResultSchema = z.object({
  ...
  local_branch: BranchNameSchema,
  local_upstream: z.string().min(1).max(300).nullable(),
  remote_name: z.string().regex(/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/),
  normalized_remote_identity: z.string().regex(/^github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/),
  configured_repository_identity: z.string().regex(/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/),

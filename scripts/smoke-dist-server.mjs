...
try {
  ...
  if (listed.tools.length !== 69 || new Set(listed.tools.map(({ name }) => name)).size !== 69) {
    throw new Error(`Built server exposed ${listed.tools.length} tools instead of 69 unique tools.`);
  }
  ...
  process.stdout.write("Built server passed health, MCP initialize, 69-tool discovery, annotations, and resource-purity smoke checks.\n");
} finally {
  if (client) {

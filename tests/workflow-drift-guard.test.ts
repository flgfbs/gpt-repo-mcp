...
describe("canonical workflow drift guards", () => {
  test("locks the intentional 69-tool surface and removed public names", () => {
    expect(toolCatalog).toHaveLength(69);
  ...
  test("keeps active workflow documentation on current public tools", async () => {
    ...
    expect(contributing).toContain("exact 69-name canonical order");
